...
ppo_model = AutoModelForCausalLMWithValueHead.from_pretrained(
    base_model_with_adapter, device_map="auto"
)
...
def length_reward(response_length: int) -> float:
    difference_ratio_min = (
        response_length - wandb.config["min_new_tokens"]
    ) / wandb.config["min_new_tokens"]
    difference_ratio_max = (
        response_length - wandb.config["max_new_tokens"]
...
def reward(batch: dict) -> list:
    emotion_scores = [
        emotion_reward(response, emotion)
...
optimizer = PagedLion32bit(
    filter(lambda p: p.requires_grad, ppo_model.parameters()),
    lr=ppo_config.learning_rate,
)
lr_scheduler = torch.optim.lr_scheduler.ExponentialLR(
    optimizer, gamma=wandb.config["lr_gamma"]
)
length_sampler = LengthSampler(
    wandb.config["min_new_tokens"], wandb.config["max_new_tokens"]
)
...
tuner.model.push_to_hub(
    repo_id="response_generator_for_emotion_chat_bot", commit="", create_pr=True
)

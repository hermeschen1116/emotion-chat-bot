...
class EmotionModel(LightningModule):
    def __init__(
        ...
        match attention:
            ...
            case "additive":
                self.__attention = AdditiveAttention(
                    dropout, dtype=dtype, device=device
                )
            case "dual_linear":
                self.__attention = DualLinearAttention(
                    dropout, dtype=dtype, device=device
                )
    ...
    def forward(
        self, representation: torch.tensor, input_emotion: torch.tensor
    ) -> torch.tensor:
        decomposed_representation: torch.tensor = representation.diag().to(
            dtype=self.__dtype, device=self.__device
        )

        output: torch.tensor = self.__attention(
            input_emotion, decomposed_representation
        ).to(dtype=self.__dtype, device=self.__device)

        attention_score: torch.tensor = torch.softmax(
            torch.sum(output, dim=1, dtype=self.__dtype), dim=0, dtype=self.__dtype
        )

        difference: torch.tensor = torch.clamp(
            torch.sum(
                self.__weight((attention_score.diag()) ** 3), dim=1, dtype=self.__dtype
            ),
            -1,
            1,
        )
    ...
    def representation_evolution(
        self, representation_src: list, emotion_compositions: list
    ) -> list:
        ...
        for composition in emotion_compositions:
            new_representation: torch.tensor = self.forward(
                torch.tensor(
                    representation[-1], dtype=self.__dtype, device=self.__device
                ),
    ...
    def training_step(self, batch, batch_idx) -> dict:
        ...
        self.__train_prediction.append(
            {"prediction": torch.tensor(prediction), "truth": torch.tensor(labels)}
        )
    ...
    def on_train_epoch_end(self) -> dict:
        all_prediction: torch.tensor = torch.cat(
            [turn["prediction"] for turn in self.__train_prediction]
        )
        all_truth: torch.tensor = torch.cat(
            [turn["truth"] for turn in self.__train_prediction]
        )

        f1_score = multiclass_f1_score(
            all_truth, all_prediction, num_classes=7, average="weighted"
        )
    ...
    def on_validation_epoch_end(self) -> dict:
        all_prediction: torch.tensor = torch.cat(
            [turn["prediction"] for turn in self.__validation_prediction]
        )
        all_truth: torch.tensor = torch.cat(
            [turn["truth"] for turn in self.__validation_prediction]
        )
        ...
        return {
            "validation_f1_score": f1_score.item(),

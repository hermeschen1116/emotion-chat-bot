...
from transformers import HfArgumentParser, TrainingArguments, AutoTokenizer, AutoModelForCausalLM, BitsAndBytesConfig
...
dataset = dataset.train_test_split(train_size=0.009)["train"]
...
# Load Tokenizer
tokenizer = AutoTokenizer.from_pretrained(wandb.config["base_model"], trust_remote_code=True)
...
special_tokens: dict = dict(zip(tokenizer.all_special_tokens, tokenizer.all_special_ids))
data_collator = DataCollatorForCompletionOnlyLM(
    special_tokens[wandb.config["response_template"]],
    instruction_template=special_tokens[wandb.config["instruction_template"]],
    tokenizer=tokenizer
)

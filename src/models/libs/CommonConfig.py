...
class CommonScriptArguments:
    def __post_init__(self):
        ...
        huggingface_hub.login(
            token=os.environ.get("HF_TOKEN", ""), add_to_git_credential=True
        )
...
class CommonWanDBArguments:
    ...
    job_type: Field[Optional[str]] = HfArg(
        aliases=["--wandb-job-type", "--job-type"], default=None
    )
    config: Field[Union[Dict, str, None]] = HfArg(
        aliases="--wandb-type", default_factory=dict
    )
    ...
    def __post_init__(self):
        ...
        self.group = value_candidate_check(
